          ...
          {if isset($item['children'])}
            {if isset($recCategory)}
              {if $recCategory['parent_id'] == $item['id']}
                <div class="collapse show" id="{$item['name_en']}-collapse">
              {else}
            ...
            {/if}
                <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small">
                  {foreach $item['children'] as $itemChild}
                    <li>
                      <a href="/?controller=categories&id={$itemChild['id']}" class="link-dark d-inline-flex text-decoration-none rounded">
                        {$itemChild['name_ru']}
                      </a>
                    </li>
                  {/foreach}
